...
void serialice_read_memory(void)
{
	u8 width;
	u32 addr;
...
void serialice_write_memory(void)
{
	u8 width;
	u32 addr;
	u32 data;
...
void serialice_write_msr(void)
{
	...
	wrmsr(addr, lo, hi, key);
}

void serialice_cpuinfo(void)
{
	u32 eax, ecx;
	u32 reg32;
...
void serialice_version(void)
{
	sio_putstring("\nSerialICE vlinuxcheck (" __DATE__ ")\n");
}
