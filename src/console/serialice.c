...
static void serialice_read_memory(void)
{
	...
	switch (width) {
	case 'b': sio_put8(read8((void *)addr)); break;
	case 'w': sio_put16(read16((void *)addr)); break;
	case 'l': sio_put32(read32((void *)addr)); break;
	}
}
...
static void serialice_write_memory(void)
{
	...
	switch (width) {
	case 'b': data = sio_get8(); write8((void *)addr, (u8)data); break;
	case 'w': data = sio_get16(); write16((void *)addr, (u16)data); break;
	case 'l': data = sio_get32(); write32((void *)addr, (u32)data); break;
	}
}
...
typedef struct { u32 lo, hi; } msrx_t;
...
{
	sio_putstring("\nSerialICE vlinuxcheck (rons hack)\n");
}
