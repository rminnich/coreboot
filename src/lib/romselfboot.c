...
static int load_self_segments(struct segment *head, int nseg,
			      struct prog *payload,
			      bool check_regions)
{
	struct segment *ptr;
...
void *selfload(struct prog *payload, bool check_regions)
{
	...
	i = build_self_segment_list(head, data, &entry);
	if (i <= 0)
		goto out;

	/* Load the segments */
	if (!load_self_segments(head, i, payload, check_regions))
		goto out;
