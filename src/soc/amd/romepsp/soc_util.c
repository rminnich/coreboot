...
static uint32_t get_internal_silicon_type(void)
{
	...
	size_t hob_size = 0;
	void /*const struct romepsp_misc_data */*hob;

	if (silicon_type)
		return silicon_type;

	hob = NULL; // fsp_find_extension_hob_by_guid(ROMEPSP_MISC_DATA_HOB_GUID.b, &hob_size);
...
static bool is_fam17_1x(void)
{
	...
	return cpuid_eax(1) >> 8 == ROMEPSP_B1_CPUID >> 8;
}
...
{
	/* only mask stepping */
	return cpuid_eax(1) >> 4 == ROMEPSP_B1_CPUID >> 4;
}
